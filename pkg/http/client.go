...
// DoWithRetry executes an HTTP request with connection pooling, automatic header rotation, and exponential backoff retries.
func (c *Client) DoWithRetry(req *http.Request, maxRetries int) (*http.Response, error) {
	...
	for i := 0; i <= maxRetries; i++ {
		// Clone request context for safe retry dial
		if i > 0 {
			select {
